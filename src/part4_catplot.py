...
def plot_nonfelony_prediction_by_charge(pred_universe_felony):
    ...
    print(" Part 4 #2 "
          "\n\n"
        " For felony rearrests, the model is much more heavily weighed by a current charge since looking at the main database tells us that way less people have a felony rearrest overall."
        "\n\n"
...
def plot_felony_prediction_by_charge_hued(pred_universe_felony):
    ...
    print(" Part 4 #3 "
        "\n\n"
        "It means that, again, the model is using a felony charge as a cudgel in the sense that if one has a current federal charge, that makes the model's propensity to predict a rearrest shoot up."
        "\n\n"
